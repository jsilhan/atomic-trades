...
from atomic_trades.conditions import (
    BalanceCondition,
    BaseCCXTCall,
    CurrencyConversion,
    PositionCondition,
...
class ExecuteOrderTillCommand(BaseCommand):
    ...
    def set_timeout(self, execute_till: Union[int, float]) -> None:
        assert (
            execute_till is None or execute_till >= 0
    ...
    async def create_limit_order_with_timeout(
        ...
        if timeout != 0:
            ...
            # some exchanges do not return bid/ask in this endpoint, use last price instead
            if ticker["ask"] and ticker["bid"]:
                spread = ticker["ask"] - ticker["bid"]
                portion_above_last_price = spread / self.LIMIT_ORDER_PORTION_FROM_SPREAD
                limit_price = (
                    ticker["bid"] + portion_above_last_price
                    if trading_side == "buy"
            ...
            if timeout is None:
                return
            if await wait_till_order_closed(
                self.exchange, order_response["id"], timeout=timeout, symbol=symbol
            ):
                return
            else:
                try:
                    await self.exchange.cancel_order(
                        id=order_response["id"], symbol=symbol
                    )
                except (OrderNotFound, InvalidOrder):
                    # order was already executed
                    return
                order_response = await self.exchange.fetch_order(
    ...
    def post_conditions(self) -> Set[BaseCCXTCall]:
        return {
            BalanceCondition(
                ...
                self.from_currency,
                "<",
                self.initial_currency_balance(self.from_currency),
            ),
            BalanceCondition(
                self.exchange,
                self.to_currency,
                ">",
                self.initial_currency_balance(self.to_currency),
            ),
        }
...
class ExchangeAllCurrency(BaseExchangeCurrencyCommand):
    ...
    async def execute(self) -> None:
        ...
        await self.create_limit_order_with_timeout(
            trading_side=trading_side, symbol=symbol, amount=amount
        )
    ...
    def __init__(
        ...
        if self.trading_symbol_conversion.is_symbol_reversed:
            self.buying_currency_conversion = CurrencyConversion(
                self.exchange,
                from_currency=self.in_currency or self.to_currency,
    ...
    def pre_conditions(self) -> Set[BaseCCXTCall]:
        ...
        if not self.margin:
            self.selling_currency_balance_condition = BalanceCondition(
                ...
                ">=",
                self.amount,
                in_currency=self.in_currency or self.to_currency,
            )
            conditions.add(self.selling_currency_balance_condition)
        return conditions

    async def execute(self) -> None:
        amount_in_buying_currency = self.buying_currency_conversion.convert_amount(
            self.amount
        )
        trading_side = (
            "sell" if self.trading_symbol_conversion.is_symbol_reversed else "buy"
        )
        await self.create_limit_order_with_timeout(
            trading_side=trading_side,
            symbol=self.trading_symbol_conversion.converting_symbol,
...
class BuyPosition(ExecuteOrderTillCommand):
    def __init__(
        ...
        self.buying_currency_conversion = CurrencyConversion(
            self.exchange,
            from_currency=in_currency or self.to_currency,
            to_currency=self.to_currency,
        )
        self.spot_usd_conversion = CurrencyConversion(
            self.exchange, from_currency="USD", to_currency=self.to_currency
        )

    def __str__(self):
        currency = self.in_currency or self.to_currency
        return f"{self.__class__.__name__}: sell {self.amount}{currency} in {self.symbol_position.symbol} position"
    ...
    async def execute(self) -> None:
        if self.in_currency:
            ...
            amount_in_buying_currency = self.buying_currency_conversion.convert_amount(
                ...
                fetch_tickers_response[self.spot_usd_conversion.converting_symbol][
                    "ask"
                ]
        ...
        await self.create_limit_order_with_timeout(
            trading_side="buy",
            symbol=self.symbol_position.symbol,
            amount=amount_in_buying_currency,
    ...
    def post_conditions(self) -> Set[BaseCCXTCall]:
        ...
        return {
            PositionCondition(
                self.exchange, self.symbol_position.symbol, ">", initial_position_amount
            )
        }
...
class SellPosition(ExecuteOrderTillCommand):
    ...
    async def execute(self) -> None:
        if self.in_currency:
            ...
            amount_in_buying_currency = self.selling_currency_conversion.convert_amount(
                self.amount
            ) * (
...
class ClosePosition(ExecuteOrderTillCommand):
    ...
    def __str__(self):
        return f"{self.__class__.__name__}: close entire {self.symbol_position_condition.symbol} position"
    ...
    async def execute(self):
        ...
        await self.create_limit_order_with_timeout(
            trading_side=trading_side,
            symbol=self.symbol_position_condition.symbol,
            amount=abs(amount),
        )
    ...
    def post_conditions(self) -> Set[BaseCCXTCall]:
        return {
            PositionCondition(
                self.exchange, self.symbol_position_condition.symbol, "==", 0
            )
        }
