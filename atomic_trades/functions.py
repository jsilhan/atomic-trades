...
def _get_aggregated_ccxt_method_calls(
    conditions_map: DefaultDict[Exchange, Set[BaseCCXTCall]],
    ...
    for exchange, conditions in conditions_map.items():
        ...
        for condition in conditions:
            ...
            merge(
                ccxt_methods_map, condition.ccxt_methods(), strategy=Strategy.ADDITIVE
            )
        ccxt_exchange_methods_map[exchange] = ccxt_methods_map
    return ccxt_exchange_methods_map


def _get_conditions_map(
    commands: Any, condition_type: str
) -> DefaultDict[Exchange, Set[BaseCCXTCall]]:
...
def _exchange_method_iterator(
    method_calls_map: Dict[Exchange, Dict[str, Dict[str, Any]]],
...
async def _get_ccxt_response(
    exchange: Exchange, method_name: str, method_params: Dict[str, Any]
...
async def _get_parallel_pre_condition_responses(
    method_calls_map: Dict[Exchange, Dict[str, Any]],
) -> List[Tuple[Exchange, str, Dict[str, Any]]]:
    tasks = [
        _get_ccxt_response(exchange, method_name, method_params)
        for exchange, method_name, method_params in _exchange_method_iterator(
            method_calls_map
        )
    ]
...
def _evaluate_conditions_from_responses(
    condition_responses_map: DefaultDict[Exchange, Dict[str, Any]],
...
async def _evaluate_conditions(
    commands: Sequence[BaseCommand], condition_type: str
) -> Tuple[Set[BaseCondition], Set[BaseCondition]]:
    ...
    successful, failed = _evaluate_conditions_from_responses(
        condition_responses_map, conditions_map
    )
    ...
    for command in commands:
        setattr(
            command,
            f"{condition_type}_condition_responses",
            condition_responses_map[command.exchange],
        )

    return successful, failed


async def _get_command_futures(
...
async def evaluate_pre_conditions(
...
async def human_execute_commands_in_sequence(
    *command_batches: Sequence[BaseCommand],
    count: int = 1,
    sleep_secs: float = 0.4,
    fail_on_first_batch_precondition: bool = False,
            ...
            while 1:
                try:
                    ...
                    if batch_count == 1 and not fail_on_first_batch_precondition:
                        print(
                            f"  {batch_count}. batch failed on pre conditions, repeating...",
                            e.failed_conditions,
                        )
                        continue
                    print(
                        f"  {batch_count}. batch failed on pre conditions:",
                        e.failed_conditions,
                    )
                    return
                except PostConditionError as e:
                    print(
                        f"  {batch_count}. batch executed and failed on post conditions:",
                        e.failed_conditions,
                    )
                    ...
                    print(
                        f"  {batch_count}. batch executed, failed during execution:",
                        e.failed_commands_dict,
                    )
                    return
