...
from atomic_trades.exceptions import (
    InvalidConditionComparatorError,
    MarketDoesNotExistsError,
)
from atomic_trades.utils import (
...
class BaseCondition(BaseCCXTCall):
    ...
    def __init__(
        ...
        exchange: Exchange,
        comparator: str,
        comparable_value: Union[int, float],
        *args,
    ) -> None:
        self.exchange = exchange
        self.comparator = (
            self.comparator_map[comparator]
            if comparator in self.comparator_map
            else comparator
        )
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            ...
            and self.comparable_value == other.comparable_value
        )
    ...
    def __hash__(self) -> int:
        return hash(
            (
                ...
                self.exchange,
                self._additional_hashable_params,
                self.comparator,
    ...
    def _compare(
        ...
        if comparator == "<":
            ...
            raise InvalidConditionComparatorError(
                f"Invalid comparator type {comparator}"
            )
...
class CurrencyConversionMixin:
    ...
    def set_from_to_currency(self, from_currency: str, to_currency: str) -> None:
        ...
        self.converting_symbol, self.is_symbol_reversed = (
            self._get_converting_symbol_and_reversed()
        )

    def _find_in_markets(self, symbol: str) -> Optional[str]:
        highest_volume = 0
        best_match = None
        for market, value in self.exchange.markets.items():
            if market.startswith(symbol) and all(not char.isdigit() for char in market):
                # ignore the future markets for currency conversion
                volume = float(value["info"].get("volume24h", 0))
    ...
    def _get_converting_symbol_and_reversed(self) -> Tuple[Optional[str], bool]:
        if self.has_same_from_to_currency():
            return None, False
        symbols = [
            (f"{self.to_currency}/{self.from_currency}", False),
            (f"{self.from_currency}/{self.to_currency}", True),
        ]
        for symbol, is_reversed in symbols:
            market = self._find_in_markets(symbol)
            if market:
                return market, is_reversed
        else:
            raise MarketDoesNotExistsError(
                f"Cannot find ticker to convert {self.from_currency} currency to {self.to_currency} currency"
            )

    def has_same_from_to_currency(self) -> bool:
        if self.from_currency.startswith("USD") and self.to_currency.startswith("USD"):
            return True
        return self.from_currency == self.to_currency

    def convert_amount(
        self, amount: Union[int, float], responses: Dict[str, Any] = None
    ) -> Union[int, float]:
        if self.has_same_from_to_currency():
            return amount
        responses = responses or self.responses
        fetched_symbol = responses["fetch_tickers"][self.converting_symbol]
        last_price = fetched_symbol["last"]
        if self.is_symbol_reversed:
            return amount * (fetched_symbol["ask"] or last_price)
        else:
            return (1 / (fetched_symbol["bid"] or last_price)) * amount


class CurrencyConversion(CurrencyConversionMixin, BaseCCXTCall):
    def __init__(
        self, exchange: Exchange, from_currency: str, to_currency: str
    ) -> None:
        self.exchange = exchange
        self.set_from_to_currency(from_currency, to_currency)

    def __str__(self):
        return f"{self.__class__.__name__}: {self.from_currency} -> {self.to_currency} on {self.exchange.name}"

    def ccxt_methods(self) -> Dict[str, Dict[str, Any]]:
        return (
            {"fetch_tickers": {"symbols": [self.converting_symbol]}}
            if not self.has_same_from_to_currency()
            else {}
        )
...
class BalanceCondition(CurrencyConversionMixin, BaseCondition):
    def __init__(
        ...
        super().__init__(
            exchange, comparator, comparable_value, currency, self.in_currency
        )
    ...
    def ccxt_methods(self) -> Dict[str, Dict[str, Any]]:
        methods: Dict[str, Dict[str, Any]] = {"fetch_balance": {}}
        if self.in_currency:
    ...
    def evaluate(self, responses: Dict[str, Any]) -> Optional[bool]:
        self.responses = responses
        self.balance_amount_in_currency = self.convert_amount(
            get_currency_balance(responses, self.currency)
        )
        return self._compare(
            self.balance_amount_in_currency, self.comparator, self.comparable_value
        )


class PositionFetch(CurrencyConversionMixin, BaseCCXTCall):
    def __init__(
        self, exchange: Exchange, symbol: str, in_currency: Optional[str] = None
    ) -> None:
    ...
    def ccxt_methods(self) -> Dict[str, Dict[str, Any]]:
        methods = {"fetch_positions": {"symbols": [self.symbol]}}
        symbols = [self.symbol]
        if (
            self.in_currency
            and self.converting_symbol
            and self.symbol != self.converting_symbol
        ):
            symbols.append(self.converting_symbol)
        methods["fetch_tickers"] = {"symbols": symbols}
        return methods

    def evaluate(self, responses: Dict[str, Any]) -> Optional[bool]:
        self.responses = responses
        self.position_amount_in_currency = self.convert_amount(
            get_position_size(responses, self.symbol)
        )
        return None
...
class PositionCondition(BaseCondition, PositionFetch):
    def __init__(
        ...
        self.in_currency = None if self.currency else in_currency
        super().__init__(
            exchange, comparator, comparable_value, symbol, self.in_currency
        )
        self.symbol = symbol
        self.set_from_to_currency(self.currency, in_currency or self.currency)
    ...
    def evaluate(self, responses: Dict[str, Any]) -> Optional[bool]:
        ...
        return self._compare(
            self.position_amount_in_currency, self.comparator, self.comparable_value
        )
